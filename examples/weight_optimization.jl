...
@time sol_bbo = Optimization.solve(opt_prob_bbo, BBO_adaptive_de_rand_1_bin_radiuslimited(); 
                                    maxiters=500, maxtime=600.0)

println("\nBlackBoxOptim Results:")
println("  Optimal weights: Wa=$(round(sol_bbo.u[1], digits=4)), " *
...
for (name, weights, ηth) in paper_results
    ...
    prob_test = qlaw_problem(oe0, oeT, tspan, μ, spacecraft; 
                             weights=weights, qlaw_params=params_test,
                             dynamics_model=dynamics_model, sun_model=sun_model, JD0=JD0)
    result = solve(prob_test)
    
    time_str = result.converged ? string(round(result.elapsed_time / 86400.0, digits=2)) : "N/A"
    mass_str = result.converged ? string(round(result.final_mass, digits=2)) : "N/A"
    
    println("$(rpad(name, 19)) | $(lpad(string(round(ηth, digits=3)), 5)) | $(lpad(time_str, 20)) | $(lpad(mass_str, 15)) | $(result.converged)")
end
println("-" ^ 85)
...
for i in 1:n_samples
    ...
    if i % 5 == 0
        println("  Evaluated $i / $n_samples samples (best so far: $(round(best_obj, digits=4)))")
    end
end

println("\nBest DOE sample: $(round(best_obj, digits=4))")
println("  Weights: Wa=$(round(samples[best_idx, 1], digits=4)), " *
...
println("  Optimal weights: Wa=$(round(sol_refined.u[1], digits=4)), " *
